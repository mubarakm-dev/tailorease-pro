...
export default async function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="p-6 max-w-3xl mx-auto flex flex-col gap-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 text-sm text-gray-900">
            ...
            <section className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
                    <h2 className="font-semibold text-sm text-gray-900">Measurements</h2>
                    {customer._count.measurements > 5 && (
                        <span className="text-xs text-gray-900">Showing latest 5 of {customer._count.measurements}</span>
                    )}
                </div>

                {customer.measurements.length === 0 ? (
                    <p className="text-sm text-gray-900 px-5 py-6 text-center">No measurements yet.</p>
                ) : (
            ...
            <section className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-900">{customer._count.orders} total</span>
                </div>
                {customer.orders.length === 0 ? (
                    <p className="text-sm text-gray-900 px-5 py-6 text-center">No orders yet.</p>
                ) : (
